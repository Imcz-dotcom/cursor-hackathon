...
import { Mic, MicOff, Zap, Activity, Grid, Sparkles as SparklesIcon } from 'lucide-react';
...
const Avatar = forwardRef(({ onCodeGenerated, viewMode, onModeChange, agentEndpoint = '/api/signed-url', agentLabel = 'Frontend Agent', isConnected: externalIsConnected }, ref) => {
    ...
    const triggerStart = async () => {
        if (viewMode === 'focus') onModeChange('chat');
        try {
    ...
    return (
        <div className="w-full h-full flex flex-col relative bg-transparent">
            ...
            <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none bg-gradient-to-br from-[#4f46e5] via-[#7c3aed] to-[#db2777]">
                ...
                <div className="h-12 border-b border-white/5 flex items-center px-4 shrink-0 bg-white/[0.02]">
                    <div className="flex-1 flex gap-3">
                        <span className="text-[10px] font-mono text-zinc-500 uppercase tracking-wider">
                            {isConnected ? 'Listening for "Frontend" keyword...' : 'Disconnected'}
                        </span>
                    </div>
